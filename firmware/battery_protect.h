...
#define D_FET							(uint32_t)(1 << 9)	// PA9	D8
#define MOSFET_MASK						(C_FET | D_FET)

// mode of BMS bit 8 and 9 indicate current status of C_FET and D_FET
#define CHARGE_MODE						(C_FET)
#define DISCHARGE_MODE					(D_FET)
#define OFF_MODE						0x00UL
#define CLOSED_MODE						(C_FET | D_FET)

// Flags are represent as bits of battery_status to indicate conditions of the battery
...
#define INT_TEMP_BELOW_0				0x04U	// bit 2 defines if the int is in -20C to -1C or 0C to 40C range (1 if below 0)
